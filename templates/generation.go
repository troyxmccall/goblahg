...
import (
	...
	"github.com/troyxmccall/goblahg/watcher"
	"io/ioutil"
	"log"
	"os"
	"path/filepath"
...
func checkThemes() error {
	...
	// If the currently set theme couldnt be compiled, try the default theme (sigh)
	err = compileTheme(filepath.Join(filenames.ThemesFilepath, "sigh"))
	if err == nil {
		// Update the theme name in the database
		err = methods.UpdateActiveTheme("sigh", 1)
		if err != nil {
			return err
		}
		return nil
