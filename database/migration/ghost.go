...
import (
	"database/sql"
	"errors"
	"github.com/troyxmccall/goblahg/filenames"
	"github.com/troyxmccall/goblahg/helpers"
	"log"
	"os"
	"path/filepath"
	"time"
...
// Function to convert a Ghost database to use with goblahg
func Ghost() {
...
// This function converts all fields in the Ghost db that are not compatible with goblahg (only date fields for now. Ghost uses a javascript-specific unix timestamp).
func convertGhostDatabase(fileName string) error {
	// If goblahg.db exists already, don't convert this file
	...
	// Convert permissions (not used by goblahg at the moment)
	err = convertDates(readDB, stmtRetrieveGhostPermissions, stmtUpdateGhostPermissions)
	if err != nil {
		log.Println("Error:", err)
		return err
	}
	// Convert clients (not used by goblahg at the moment)
...
func convertUsers(readDB *sql.DB) error {
	...
	for rows.Next() {
		...
		// Convert strings to byte array since that is how goblahg saves the user name and email (login won't work without this).
		if name.Valid {
...
func setDefaultTheme(readDB *sql.DB) error {
	...
	// Update the database with the default theme (sigh)
	date := time.Now()
	_, err = writeDB.Exec(stmtUpdateGhostTheme, "sigh", date, 1)
	if err != nil {
		writeDB.Rollback()
		return err
	}
