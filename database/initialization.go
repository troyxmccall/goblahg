...
import (
	"database/sql"
	"github.com/troyxmccall/goblahg/database/migration"
	"github.com/troyxmccall/goblahg/filenames"
	"github.com/troyxmccall/goblahg/helpers"
	"github.com/troyxmccall/goblahg/structure"
...
var stmtInitialization = `CREATE TABLE IF NOT EXISTS
	...
	INSERT OR IGNORE INTO settings (id, uuid, key, value, type, created_at, created_by, updated_at, updated_by) VALUES (7, ?, 'activeTheme', 'sigh', 'theme', ?, 1, ?, 1);
...
func Initialize() error {
	// If goblahg.db does not exist, look for a Ghost database to convert
	if !helpers.FileExists(filenames.DatabaseFilename) {
		// Convert Ghost database if available (time format needs to change to be compatible with goblahg)
		migration.Ghost()
	}
...
func checkBlogSettings() error {
	...
	if err != nil {
		// Insert activeTheme
		err = insertSettingString("activeTheme", "sigh", "theme", time.Now(), 1)
		if err != nil {
			return err
		}
	}
