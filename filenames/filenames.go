...
var (
	// Determine the path the goblahg executable is in - needed to load relative assets
	ExecutablePath = determineExecutablePath()

	// Determine the path the the assets folder (default: goblahg root folder)
	AssetPath = determineAssetPath()

	// For assets that are created, changed, our user-provided while running goblahg
	...
	DatabaseFilename = filepath.Join(ContentFilepath, "data", "goblahg.db")
